...
public class NFCActivity extends AppCompatActivity {
    ...
    //当获取到NfcV标签
    protected void onNfcV(String id,NfcV tag){
        try {
            tag.connect();
            byte[] idb = tag.getTag().getId();
            byte cmd[] = new byte[10];
            cmd[0] = (byte) 0x22;
            cmd[1] = (byte) 0x2B;
            System.arraycopy(idb, 0, cmd, 2, idb.length); // UID
    ...
    protected void onNewIntent(Intent intent) {
        ...
        for(String v : techList){
            if(v == NfcV.class.getName()){
                tech.vtag = NfcV.get(tag);
                if(tech.vtag != null){
                    onNfcV(id,tech.vtag);
                }
